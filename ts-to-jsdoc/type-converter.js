// @ts-nocheck
/**
 * source code was taken from better-docs jsdoc plugin for typescript
 *      - see the repo for more information https://github.com/SoftwareBrothers/better-docs
 */
...
const getName = (node, src) => {
  const name =
    (node.name && node.name.escapedText) ||
...
const convertParams = (jsDoc = '', node, src, parentName = null) => {
  node.type.parameters.forEach((parameter) => {
    ...
    const comment = (parameter.jsDoc && parameter.jsDoc[0] && parameter.jsDoc[0].comment) || '';
 ...
 */
// @ts-ignore
const convertMembers = (jsDoc = '', type, src, parentName = null) => {
  ...
  typesToCheck.forEach((type) => {
    ...
    if (type.typeName && type.typeName.escapedText === 'Array') {
      if (type.typeArguments && type.typeArguments.length) {
        type.typeArguments.forEach((subType) => {
          jsDoc = convertMembers(
            jsDoc,
            subType,
            src,
            parentName ? parentName + '[]' : '' // when there is no parent - jsdoc cannot parse [].name
          );
        });
      }
    }
    ...
    (type.members || [])
      .filter((m) => ts.isTypeElement(m))
      .forEach((member) => {
        let name = getName(member, src);
        const comment = (member.jsDoc && member.jsDoc[0] && member.jsDoc[0].comment) || '';
...
module.exports = function typeConverter(src, filename = 'test.ts') {
  const ast = ts.createSourceFile(path.basename(filename), src, ts.ScriptTarget.Latest, false, ts.ScriptKind.TS);
      ...
      if (jsDocNode) {
        ...
        if (ts.isInterfaceDeclaration(statement)) {
          ...
          statement.members.forEach((member) => {
            ...
            memberComment = appendComment(memberComment, [`@name ${name}#${memberName}`].join('\n'));
